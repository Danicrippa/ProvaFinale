        ...
        private static Giocatore ScegliEroe(Utente utente)
        {
            ...
            if (eroi.Count > 0)
            {
                ...
                do
                {
                    ...
                    {
                        Console.WriteLine($"Premi {count} per scegliere eroe {eroe.Nome}, tipo {CategoriaEroe.Guerriero} " +
                                $"  arma {eroe.Arma} con {eroe.Arma.PuntiDanno} punti danno,  livello {eroe.Livello} e vita {eroe.PuntiVita}");
                        count++;
                    }
